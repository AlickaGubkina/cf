...
async function addtolib(fic,lib){return await fetch("https://ficbook.net/ajax/collections/addfanfic", {
  "headers": {
    "accept": "*/*",
    "accept-language": "be-BY,be;q=0.9",
    "cache-control": "no-cache",
  ...
  "mode": "cors",
  "credentials": "include"
}).then((x)=>{if(x.status==404){throw -1;}if(x.status==429){throw -2;}x.json();}).then((w)=>{return w});}
...
async function q(login,pass,texts,pberr,pbs,pbe, TOO_MANY_REQUEST_DELAY=7000){
  ...
  while(true){
    try{//сборники
       await randel(libs).addfic(getficid());
    }
    catch(ex){
      if(ex==-1){
        //несуществующий фф
    ...
    try{//комменты
       await comm(getpartid(), randel(texts));
    }
    catch(ex){
      if(ex==-1){
    ...
    try{//пб
       await pb(getpartid(), randel(texts), randel(pberr), randel(pbs),randel(pbe));
    }
    catch(ex){
      if(ex==-1){
        //несуществующий фф\часть
